...
def PRODUCT():
    return Model_ConfigProduct.find_one_by_attr({"config_product_code": "CI21000"})
...
def load(hostname: str, *args, **kwargs) -> None:
    ...
    url = urljoin(
        hostname,
        f"api/config/product/{product.config_product_id}/relationship-mapper/sets",
    )
    res = requests.post(url, json=DATA_RELATIONSHIP_MAPPER(product), **kwargs)
    if not res.ok:
