...
class Model_ConfigProvisionUI(BaseModel):
    ...
    __table_args__ = (
        db.UniqueConstraint('config_provision_id'),
    )

    config_provision_id = db.Column(db.ForeignKey(f"{CONFIG_PROVISION}.config_provision_id"), primary_key=True)
...
class Model_ConfigProvisionUI_Input(Model_ConfigProvisionUI):
    ...
    input_type = db.relationship("Model_RefInputTypes", 
