...
class Model_ConfigBenefitAuth(BaseModel):
    ...
    __table_args__ = (db.UniqueConstraint("config_benefit_id", "priority"),)
    ...
    acl = db.relationship("Model_ConfigBenefitAuth_ACL", innerjoin=True, lazy="joined")


class Model_ConfigBenefit(BaseModel):
    __tablename__ = CONFIG_BENEFIT
    __table_args__ = (db.UniqueConstraint("config_product_id", "config_benefit_code"),)
    ...
    unit_type = db.relationship(
        "Model_RefUnitCode",
        primaryjoin="Model_ConfigBenefit.unit_type_id == Model_RefUnitCode.ref_id",
        lazy="joined",
    )
    benefit_auth = db.relationship(
