...
class Progress_ConfigProduct(Resource): 
    ...
    def _progress_tracker(cls, product: Model_ConfigProduct, requirements: List[str]): 
        completed = sum([1 for r in requirements if getattr(product, r, None) is not None])
    ...
    def get(cls, id):
        ...
        prog_dists = cls._progress_tracker(product, 
            ['gender_distribution_set_id', 'smoker_status_distribution_set_id', 'age_distribution_set_id'])
            
        prog_strategies = cls._progress_tracker(product, 
            ['gender_rating_strategy_id', 'smoker_status_rating_strategy_id', 'age_rating_strategy_id'])
