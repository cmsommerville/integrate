...
const SelectSmokerStatus = ({ plan_id, ...props }: IProps) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/crud/selection/plan/${plan_id}`, { signal })
      ...
      .then((res) => {
        setSelection({
          id: res.smoker_status_mapper_set.config_product_mapper_set_id,
          label: res.smoker_status_mapper_set.mapper_type_label,
        });
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/data/selection/plan/${plan_id}/smoker-status-mapper-list`, {
      signal,
    })
      .then((res) => res.json())
      .then((res) => {
  ...
  const _onChange = (val: IAppSelectOption) => {
    ...
    fetch(`/api/crud/selection/plan/${plan_id}`, {
      method: "PATCH",
      body: JSON.stringify({
        config_smoker_status_product_mapper_set_id: val.id,
      }),
      headers: {
...
export default SelectSmokerStatus;
