    ...
    def post(cls):
        try:
            ...
            if user_name is None:
                return {"status": "error", "message": "Please provide user name"}, 400

            roles = data.get('roles')
            if not isinstance(roles, list):
                return {"status": "error", "message": "Please provide a list of roles"}, 400
            if len(roles) == 0: 
                return {"status": "error", "message": "Please provide at least one role"}, 400

            user = Model_AuthUser.find_by_user_name(user_name)
            if user is None:
                return {"status": "error", "message": f"Cannot find user with user name {user_name}"}, 400
            ...
            return {"status": "success", "message": "Added roles to user"}, 201
...
class Resource_AdminRemoveUserRole(Resource):
    ...
    def post(cls):
        try:
            ...
            return {"status": "success", "message": "Removed requested roles from user"}, 201
