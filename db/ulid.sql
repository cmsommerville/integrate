...
ALTER FUNCTION [dbo].[ulid] () RETURNS UNIQUEIDENTIFIER AS 
BEGIN
...
ALTER FUNCTION [dbo].[base32CrockfordEnc] (@x VARBINARY(max), @pad INT = 1) RETURNS VARCHAR(max) AS BEGIN
...
ALTER FUNCTION [dbo].[base32CrockfordDec] (@x VARCHAR(max)) RETURNS VARBINARY(max) AS BEGIN
...
ALTER FUNCTION [dbo].[ulidStr] () RETURNS VARCHAR(100) AS BEGIN
...
ALTER FUNCTION [dbo].[ulid_seeded] (@dt DATETIME2, @rnd BINARY (10)) RETURNS UNIQUEIDENTIFIER AS BEGIN
DECLARE @di BIGINT
SET @di = DATEDIFF(
        hour,
        CAST('1970-01-01 00:00:00' AS DATETIME2),
