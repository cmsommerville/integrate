...
def inject_defaults(objs):
    for obj in objs:
        setattr(obj, "updated_by", "sys")
    return objs


def load_refdata():
    objs = schemas.Schema_RefAttrMapperType(many=True).load(DATA_ATTR_MAPPER_TYPE)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefCensusStrategy(many=True).load(DATA_CENSUS_STRATEGY)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefComparisonOperator(many=True).load(
        DATA_COMPARISON_OPERATOR
    )
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefDataTypes(many=True).load(DATA_DATA_TYPES)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefInputTypes(many=True).load(DATA_INPUT_TYPES)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefOptionality(many=True).load(DATA_OPTIONALITY)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefPlanStatus(many=True).load(DATA_PLAN_STATUS)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefPremiumFrequency(many=True).load(DATA_PREMIUM_FREQUENCIES)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefRateFrequency(many=True).load(DATA_RATE_FREQUENCIES)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefRatingStrategy(many=True).load(DATA_RATING_STRATEGY)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefStates(many=True).load(REF_STATES)
    objs = inject_defaults(objs)
    db.session.add_all(objs)

    objs = schemas.Schema_RefUnitCode(many=True).load(DATA_UNIT_TYPE)
    objs = inject_defaults(objs)
    db.session.add_all(objs)
...
def downgrade_refdata():
    op.execute("DELETE FROM ref_states")
    op.execute("DELETE FROM ref_master")
