...
GO ALTER FUNCTION [dbo].[ulid] () RETURNS UNIQUEIDENTIFIER AS BEGIN
...
SET @di = (@di * 1000) + DATEPART(ms, @dt) RETURN CAST(
        @rnd + SUBSTRING(CAST(@di AS BINARY (8)), 3, 6) AS UNIQUEIDENTIFIER
    )
END
GO
GRANT EXECUTE ON [dbo].[ulid] TO PUBLIC
GO IF NOT EXISTS (
        SELECT *
        FROM sys.objects
...
GO ALTER FUNCTION [dbo].[base32CrockfordEnc] (@x VARBINARY(max), @pad INT = 1) RETURNS VARCHAR(max) AS BEGIN
...
SET @c = 0 WHILE @c < DATALENGTH(@x) BEGIN
SET @s = 0
SET @t = CAST(SUBSTRING(@x, @c + 1, 5) AS BIGINT)
SET @op = '' WHILE @s < 8 BEGIN
SET @q = @t % 32
...
SET @c = @c + 5
END
DECLARE @padc CHAR(1) --padding section
SET @padc = CASE
        WHEN @pad IS NULL
        OR @pad = 1 THEN '='
        ELSE ''
    END
SET @o = CASE
        WHEN @p = 1 THEN SUBSTRING(@o, 1, DATALENGTH(@o) - 6) + REPLICATE(@padc, 6)
        WHEN @p = 2 THEN SUBSTRING(@o, 1, DATALENGTH(@o) - 4) + REPLICATE(@padc, 4)
        WHEN @p = 3 THEN SUBSTRING(@o, 1, DATALENGTH(@o) - 3) + REPLICATE(@padc, 3)
        WHEN @p = 4 THEN SUBSTRING(@o, 1, DATALENGTH(@o) - 1) + REPLICATE(@padc, 1)
        ELSE @o
    END RETURN LTRIM(RTRIM(@o))
END
GO
GRANT EXECUTE ON [dbo].[base32CrockfordEnc] TO PUBLIC
GO IF NOT EXISTS (
        SELECT *
        FROM sys.objects
...
GO ALTER FUNCTION [dbo].[base32CrockfordDec] (@x VARCHAR(max)) RETURNS VARBINARY(max) AS BEGIN
...
SET @x = REPLACE(@x, 'I', '1')
SET @x = REPLACE(@x, 'O', '0')
SET @c = 1 WHILE @c < DATALENGTH(@x) + 1 BEGIN
SET @s = 0
SET @t = 0 WHILE @s < 8 --accumulate 8 characters (40 bits) at a time in a bigint
    BEGIN
SET @t = @t * 32
...
END
GO
GRANT EXECUTE ON [dbo].[base32CrockfordDec] TO PUBLIC
GO IF NOT EXISTS (
        SELECT *
        FROM sys.objects
...
GO ALTER FUNCTION [dbo].[ulidStr] () RETURNS VARCHAR(100) AS BEGIN
DECLARE @temp BINARY (16)
SET @temp = CAST(dbo.ulid() AS BINARY (16)) RETURN [dbo].[base32CrockfordEnc](
        SUBSTRING(@temp, 11, 6) + SUBSTRING(@temp, 1, 10),
        0
    )
END
GO
GRANT EXECUTE ON [dbo].[ulidStr] TO PUBLIC
GO IF NOT EXISTS (
        SELECT *
        FROM sys.objects
...
GO ALTER FUNCTION [dbo].[ulid_seeded] (@dt DATETIME2, @rnd BINARY (10)) RETURNS UNIQUEIDENTIFIER AS BEGIN
DECLARE @di BIGINT
SET @di = DATEDIFF(
        hour,
        CAST('1970-01-01 00:00:00' AS DATETIME2),
